        ...
        sellData.items.forEach(item => {
          ...
          if (itemId === currentId && Array.isArray(item.batches)) {
            item.batches.forEach(b => {
              if (b.batch) {
                // If the item in cart is in grams, we need to convert its batch allocation to kg for the dialog
                const itemIsGram = item.unit === 'gram' || item.unit === 'g';
                let allocatedQty = parseFloat(b.quantity) || 0;

                // Note: stored items should ideally have batches stored in base unit (kg), 
                // but if we are storing batches in grams for gram items, we interpret it here.
                // Based on our new logic, we will store batches in KG always, but let's be safe.
                // Actually, let's assume batches in item.batches are always in base unit (kg) per our new design.

                preAllocated[b.batch] = (preAllocated[b.batch] || 0) + allocatedQty;
              }
            });
          }
        });
    ...
    try {
      ...
      const payload = {
        ...
        items: currentItems.map(item => {
          // Normalize quantity for backend (convert grams to kg)
    ...
    try {
      ...
      const sendSaleData = async (salePayload) => {
        try {
          ...
          if (data.success) {
            showSuccess(`Sale created successfully! Bill Number: ${data.bill_number || data.sale_id}`);
            navigate('/caterers');
          } else {
      ...
      // Create JSON payload for API (bill_number will be generated by backend)
      const payload = {
        caterer_id: sellData.caterer_id,
        sell_date: sellData.sell_date,
        items: currentItems,
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {batchSelectionDialog.isOpen && batchSelectionDialog.product && (
        <CatererBatchSelectionDialog
          ...
          initialQuantity={batchSelectionDialog.initialQuantity}
        />
      )}
      ...
      {mixProductSelectionDialog.isOpen && (
        <MixProductSelectionDialog
          ...
          onBatchSelectionComplete={(selectedMixData) => {
            ...
            // Normalize mix items for backend if they are in grams (unlikely for mix calc but safe to have)
            const normalizedItems = newItems.map(item => {
              // Mix items are typically in kg/base unit from calculator, so no change needed usually
              // But if we ever support grams in mix calc, we'd process here.
              return item;
            });
            ...
            setSellData(prev => ({
              ...prev,
              items: [...prev.items, mixHeaderItem, ...normalizedItems]
            }));
